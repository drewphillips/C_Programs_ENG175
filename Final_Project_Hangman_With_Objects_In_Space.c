...
Additional improvements, The loop again menu was changed up to include an option to see all the words.
...
and call the cheat sheet option, This was intentional, I'll be sending this to friends and family.

FIXME Don't add the correct letter to wrongLetter array. I tried a few ways to accomplish this, and that code is still in here.
...
int processguess(char Challenge[], char Gword[], int WordLen, char GuessedLetter);   /* compares guessed letter with word to be guessed
...
void drawfig(const int, char wrongLetters[26], int WordLen);    // draws hangman figure based on the number of wrong guesses by the user
...
/*this is the code to understand, and clean up.

// new prototype:
// int processguess(char Challenge[], char Gword[], int WordLen, char GuessedLetter char WrongLetters);
prototype:  int processguess(char[], char[], int, char);     compares guessed letter with word to be guessed
                                                --if guess is correct, letter is placed in user's word array;
                                                otherwise, wrong guesses are counted
Call:  if (!processguess(challenge, gword, wordLen, guess, wrongLetters)) // why !not process challange.
                wrongs++;
                int processguess(char c[], char g[], int l, char gl)
*/
int processguess(char Challenge[], char Gword[], int WordLen, char GuessedLetter) {
    int i, j, hits = 0;
    for (i = 0; i <= WordLen; i++) //run throw the challange word array
    {
        if (Challenge[i] == GuessedLetter)  //if index of challeng word compairs to the guessed letter, 
        {
            Gword[i] = GuessedLetter;  //set the corect letters array to the leter guessed.  
            hits++;   // what does hts do? it returns to what?
        }
        //else WrongLetters[j] = GuessedLetter;
        //j = j+1;
        //printf(" %s", WrongLetters);
    }
